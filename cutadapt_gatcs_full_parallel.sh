...
cat $len9/output0-gatcs.fastq $len9/sed_output*-gatcs.fastq $olen/output0-gatcs.fastq $olen/sed_output*-gatcs.fastq > $basef/summary_gatcs_${fq_base}.fastq
#cat $olen/output0-gatcs.fastq $olen/sed_output*-gatcs.fastq > $olen/untrim_${fq_base}.fastq

s4_summary_gatcs=$((`wc -l $basef/summary_gatcs_${fq_base}.fastq | sed 's/[a-zA-Z0-9_./-]*$//'`/4))
s4_summary_gatcs_pct=`bc <<< "scale=4; a=$s0_reads; b=$s4_summary_gatcs; (b/a)*100" | sed 's/[0].$//'`%

s4_trash_reads=$((${s3_input_trim_reads}+${s3_input_untrim_reads}+${s2_trash_reads}-${s3_match_trim_reads}-${s3_match_untrim_reads}))
s4_trash_reads_pct=`bc <<< "scale=4; a=$s0_reads; b=$s4_trash_reads; (b/a)*100" | sed 's/[0].$//'`%
...
<div class=\"span4\">
<h4 align=\"center\"><script>document.write(number_format(${s2_untrim}, 0, '.', ' '))</script> reads (${s2_untrim_pct})</h4>
<p align=\"center\">without GATC(s), original length</p>
</div>
<div class=\"span4\" style=\"background-color: #448f30\">
<h4 align=\"center\"><script>document.write(number_format(${s4_summary_gatcs}, 0, '.', ' '))</script> reads (${s4_summary_gatcs_pct})</h4>
<p align=\"center\">with edge GATC(s)</p>
</div>
<div class=\"span4\" style=\"color: #f10026\">
<h4 align=\"center\"><script>document.write(number_format(${s4_trash_reads}, 0, '.', ' '))</script> reads (${s4_trash_reads_pct})</h4>
